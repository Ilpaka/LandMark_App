...
final followApiProvider = Provider((ref) => FollowApi(ref.read(apiClientProvider).dio));

// Follow stats for a given userId (autoDispose so it refreshes on re-entry)
final followStatsProvider = AsyncNotifierProviderFamily<FollowStatsNotifier, FollowStats, String>(
...
final publicProfileProvider =
    FutureProvider.autoDispose.family<UserProfile, String>((ref, userId) =>
        ref.read(followApiProvider).getPublicProfile(userId));

// Followers list
final followersProvider =
    FutureProvider.autoDispose.family<List<UserProfile>, String>((ref, userId) =>
        ref.read(followApiProvider).listFollowers(userId));

// Following list
final followingProvider =
    FutureProvider.autoDispose.family<List<UserProfile>, String>((ref, userId) =>
        ref.read(followApiProvider).listFollowing(userId));
